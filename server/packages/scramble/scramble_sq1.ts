...
import { invertAlg } from "../rubiks-viz";
...
function FullCube_getShapeIdx(obj) {
    const magic = 0b100010001000100010001;
    var dlx, drx, ulx, urx;
    urx = obj.ur & magic;
    urx |= urx >> 3;
    urx |= urx >> 6;
...
function FullCube_getSquare(obj, sq) {
    ...
    sq.cornperm = mathlib.get8Perm(obj.prm);
    ...
    for (; b < 8; a += 3, ++b)
        obj.prm[b] = FullCube_pieceAt(obj, a) >> 1;
    sq.edgeperm = mathlib.get8Perm(obj.prm);
    sq.ml = obj.ml;
}
...
function Search_init2(obj) {
    ...
    for (i = prun; i < obj.Search_maxlen2; ++i) {
        if (Search_phase2(obj, edge, corner, obj.Search_sq.topEdgeFirst, obj.Search_sq.botEdgeFirst, ml, i, obj.Search_length1, 0)) {
            ...
            obj.Search_sol_string = movesToStr(obj.Search_move, i + obj.Search_length1);
            return true;
        }
    }
    return false;
...
export function movesToStr(moves, len) {
    ...
    for (var i = len - 1; i >= 0; i--) {
        var val = moves[i];
        // > 0 is a top move, < 0 is a bottom move, 0 is a slash
        if (val > 0) {
            // val = 12 - val;
...
export function strToMoves(str) {
    ...
    str
        ...
        .forEach(chunk => {
            ...
            const [top, bottom] = chunk
                .split(",")
                .map(x => parseInt(x));
            if (top) {
                top < 0 ? moves.push(-top) : moves.push(12 - top);
            }
            if (bottom) {
                bottom < 0 ? moves.push(bottom) : moves.push(bottom - 12);
            }
        });
    return moves;
}
...
function Search_solution(obj, c) {
    obj.Search_c = c;
    var shape = FullCube_getShapeIdx(c);
...
function Shape_getShape2Idx(shp) {
    const ones_24 = 0b111111111111111111111111;
    return binarySearch(Shape_ShapeIdx, shp & ones_24) << 1 | shp >> 24;
}
...
function Square_init() {
    ...
    pos = [];
    for (i = 0; i < 40320; ++i) {
        mathlib.set8Perm(pos, i);
        mathlib.circle(pos, 2, 4)(pos, 3, 5);
        Square_TwistMove[i] = mathlib.get8Perm(pos);
        mathlib.set8Perm(pos, i);
        mathlib.circle(pos, 0, 3, 2, 1);
        Square_TopMove[i] = mathlib.get8Perm(pos);
        mathlib.set8Perm(pos, i);
        mathlib.circle(pos, 4, 7, 6, 5);
        Square_BottomMove[i] = mathlib.get8Perm(pos);
    }
    for (i = 0; i < 80640; ++i) {
        SquarePrun[i] = -1;
    }
